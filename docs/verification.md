...
| --- | --- |
| `cargo build` | Passed |
| `cargo test` | 74 passed; feature-gated live target excluded |
| `cargo test --locked --all-features` | 76 offline tests passed; 3 live cases ignored |
| `cargo fmt --all -- --check` | Passed |
| `cargo clippy --all-targets --all-features -- -D warnings` | Passed |
| `RUSTDOCFLAGS='-D warnings' cargo doc --no-deps` | Passed |
| `cargo +1.88.0 build --locked` | Passed |
| `cargo +1.88.0 test --locked --all-features` | 76 offline tests passed; 3 live cases ignored |
...
| Real Chrome/Keychain and authenticated GraphQL | User-reported post/parent/reply smoke stages passed; search failed with exit 8, timeline not reached. No browser/Keychain access by the agent |
